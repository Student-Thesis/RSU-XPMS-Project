                    ...
                    <div class="tab-pane fade show active" id="proposals" role="tabpanel">
                        ...
                        @if ($pendingProposals->count() > 0)
                            <div class="table-responsive">
                                            ...
                                            <tr>
                                                ...
                                                <td>
                                                    <button class="btn btn-sm btn-info viewProposalBtn"
                                                        data-id="{{ $proposal->id }}" data-bs-toggle="modal"
                                    ...
                                    <div class="modal-dialog modal-lg modal-dialog-centered">
                                        <div class="modal-content">
                                            ...
                                            <div class="modal-body">
                                                ...
                                                <div class="row mb-2">
                                                    ...
                                                    <div class="col-md-6">
                                                        ...
                                                        <small class="text-muted">Date Signed: <span
                                                                id="vp_date_signed"></span></small>
                                                    </div>
                                                </div>
                                            ...
                                            <div class="modal-footer">
                                                ...
                                                <button class="btn btn-danger rejectProposalBtn"
                                                    data-id="{{ $proposal->id }}">
                                ...
                                <script>
                                    ...
                                    document.querySelectorAll('.viewProposalBtn').forEach(btn => {
                                        btn.addEventListener('click', async function() {
                                            ...
                                            document.getElementById('vp_files').innerHTML =
                                                '<span class="text-muted">Loading attachments…</span>';
                                            document.getElementById('vp_flags').innerHTML =
                                                '<span class="text-muted">Loading flags…</span>';
                                            ...
                                            try {
                                                ...
                                                document.getElementById('vp_budget').textContent = Number(p.approved_budget || 0)
                                                    .toLocaleString(undefined, {
                                                        minimumFractionDigits: 2
                                                    });
                                                document.getElementById('vp_description').textContent = p.description ||
                                                    'No description';
                                                ...
                                                // attachments
                                                const files = [];
                                                if (p.documentation_url) files.push({
                                                    label: 'Documentation Report',
                                                    url: p.documentation_url
                                                });
                                                if (p.mou_url) files.push({
                                                    label: 'MOU File',
                                                    url: p.mou_url
                                                });
                                                if (p.moa_url) files.push({
                                                    label: 'MOA File',
                                                    url: p.moa_url
                                                });
                                                if (p.drive_link) files.push({
                                                    label: 'Drive Link',
                                                    url: p.drive_link
                                                });
                                                if (p.moa_link) files.push({
                                                    label: 'MOA Link',
                                                    url: p.moa_link
                                                });
                                                ...
                                                vpFlags.innerHTML = trueFlags.length ?
                                                    trueFlags.map(k =>
                                                        `<span class="badge bg-success">${escHtml(flagsMap[k])}</span>`).join(' ') :
                                                ...
                                                document.getElementById('vp_files').innerHTML =
                                                    '<span class="text-danger">Failed to load attachments.</span>';
                                                document.getElementById('vp_flags').innerHTML =
                                                    '<span class="text-danger">Failed to load flags.</span>';
                                            }
                                        });
                                    });
                                ...
                                <script>
                                    // ❌ REJECT HANDLER (FIXED)
                                    document.querySelectorAll('.rejectProposalBtn').forEach(btn => {
                                        btn.addEventListener('click', function() {
                                            ...
                                            Swal.fire({
                                                ...
                                                text: "This proposal will be rejected and cannot be undone.",
                                                icon: "warning",
                                                showCancelButton: true,
                                                ...
                                                // ✅ DEFINE URL HERE
                                                const url = "{{ route('proposals.reject', ':id') }}".replace(':id', id);
                                                ...
                                                fetch(url, {
                                                        ...
                                                        body: JSON.stringify({}) // optional, but keeps it consistent
                                                    })
                                                    .then(async res => {
                                                        let json = {};
                                                        try {
                                                    ...
                                                    .then(json => {
                                                        ...
                                                        Swal.fire({
                                                            title: "Rejected",
                                                            text: json.message || "Proposal rejected successfully.",
                                                            icon: "success",
                                                            confirmButtonColor: "#dc3545"
                                                        }).then(() => window.location.reload());
                                                    })
                                                    .catch(err => {
                                                        Swal.fire({
                                                            title: "Error",
                                                            text: err?.message || "Failed to reject proposal.",
                                                            icon: "error",
                                                            confirmButtonColor: "#dc3545"
                                                        });
                                                    });
