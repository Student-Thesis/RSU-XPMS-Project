...
class AgreementController extends Controller
{
    ...
    public function store(Request $request)
    {
        try {
            ...
            if (!$userId) {
                return back()
                    ->withInput()
                    ->with('error', 'User information is missing. Please register or login again.');
            }
            ...
            if (!$hasAnyData) {
                ...
                try {
                    ...
                    if ($user) {
                        $adminBody =
                            "A new user account has been registered (no agreement data submitted).\n\n" .
                ...
                return redirect()
                    ...
                    ->with('success', 'Your account has been created. You may submit agreement documents later.');
            }
            ...
            // ✅ Move files directly to public/uploads/agreements
            if ($request->hasFile('mouFile')) {
                $payload['mou_path'] = $this->moveToPublic(
                    $request->file('mouFile'),
                    'uploads/agreements'
                );
            }

            if ($request->hasFile('moaFile')) {
                $payload['moa_path'] = $this->moveToPublic(
                    $request->file('moaFile'),
                    'uploads/agreements'
                );
            }
            ...
            try {
                ...
                // Admin email
                $adminBody =
                    "A new agreement has been submitted.\n\n" .
                    "User: {$userLine}\n" .
                    "User ID: {$userId}\n" .
                    "Proposal ID: {$proposalId}\n" .
                ...
                if ($user && $user->email) {
                    ...
                    Mail::raw($userBody, function ($message) use ($user) {
                        $message->to($user->email, $user->first_name . ' ' . $user->last_name)
                            ->subject('Your Agreement Submission Has Been Received');
                    });
    ...
    protected static function uniqueName(UploadedFile $file): string
    {
     ...
     * Returns a web-servable relative path like: uploads/agreements/abc.pdf
     */
    protected function moveToPublic(UploadedFile $file, string $folder = 'uploads/agreements'): string
    {
        ...
        // Store relative to public so you can use asset($path)
